...
GRID_COL_OFFSET = 0.085           # lateral offset of each column from centerline
...
def build_geometry_boxes(res, wall_step=0.14, surface_step=0.35):
    ...
    if branches:
        ...
        branch_wall_gaps = []  # parallel to `branches`: [(lo0,hi0), (lo1,hi1)]
        for br in branches:
            ...
            start_reach = start_info["branch_s_reach"] if start_info else 0.0
            end_reach = end_info["branch_s_reach"] if end_info else 0.0
            branch_wall_gaps.append([(0.0, start_reach), (br["length"] - end_reach, br["length"])])

        out["walls_main_left"] = build_ribbon_boxes_with_gaps(lw_dec, wt, gaps_left, closed=True)
        out["walls_main_right"] = build_ribbon_boxes_with_gaps(rw_dec, wt, gaps_right, closed=True)
        ...
        for i, br in enumerate(branches):
            ...
            barr = br["arr"]
            bw = br["width_m"]
            gaps_i = [g for g in branch_wall_gaps[i] if g[1] > g[0]]
            bwall_l = decimate_by_arclength(offset_polyline(barr, bw / 2 + wt / 2), wall_step, closed=False)
            bwall_r = decimate_by_arclength(offset_polyline(barr, -(bw / 2 + wt / 2)), wall_step, closed=False)
            out[f"walls_{key}_left"] = build_ribbon_boxes_with_gaps(bwall_l, wt, gaps_i, closed=False)
            out[f"walls_{key}_right"] = build_ribbon_boxes_with_gaps(bwall_r, wt, gaps_i, closed=False)
