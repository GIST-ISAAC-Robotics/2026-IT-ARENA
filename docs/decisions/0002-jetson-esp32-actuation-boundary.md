...
2026-09-22 A1-2 후속: **선택형 ROS/가상 MCU 연결**과 정지/복구 검증을 마쳤다. [현재 연결 구조와 시험 한계](../hardware/ACTUATION_ROS_2026_09_22.md)를 우선한다. 아래 A1-1 시점의 ‘ROS 연결은 아직’은 당시 상태다. 실제 MCU 펌웨어/ESC·내부 엔코더 속도 루프는 아직이며 기본 실행은 legacy로 보존한다.

2026-09-22 후속: 사용자가 **ESP32-S3 우선 후보·MCU 엔코더 기반 모터 속도 제어**를 다시 확인했다. 보드 없는 구동/정지 계약의 참조 구현·시험은 [ADR 0023](0023-mcu-actuation-contract-reference.md)과 [구동 계약 보고서](../hardware/ACTUATION_CONTRACT_2026_09_22.md)를 우선한다. 실제 펌웨어/ESC·서보 어댑터·ROS 연결은 아직이며 아래 인터페이스 도식의 서보 PWM을 확정 사양으로 해석하지 않는다. 현재 새 주행 모드는 ToF 대신 독립 LiDAR 보호층을 사용한다.
