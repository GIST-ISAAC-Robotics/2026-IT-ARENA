...
직선이 10.3 m라고 깊이 카메라의 유효 거리를 10.3 m로 늘리지는 않습니다. RGB의 렌더링 거리를 깊이 범위와 분리하고, 실제 장애물 회피 요구 거리는 `속도 × 총 지연 + 속도² / (2 × 유효 감속도) + 여유 거리`를 출발점으로 정합니다. 감속도·타이어·노면과 센서 오차는 실측이 필요합니다. 이후 주변 차량 감지 구조는 [상부 C1급 라이다와 하부 ToF](TOF_RING.md)로 정하고 ToF 여섯 개를 임시 모델에 배치했습니다. D435i는 보유 장비이므로 RGB·깊이 비교를 위해 유지하지만, 깊이를 최종 주행 입력으로 쓸지 또는 소형 RGB 카메라로 바꿀지는 아직 확정하지 않았습니다.
