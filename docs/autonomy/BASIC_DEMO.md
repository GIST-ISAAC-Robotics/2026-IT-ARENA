...
데모 제어기는 처리 부담을 줄이기 위해 **RGB 848×480·30 Hz, 라이다 10 Hz**를 사용하고 깊이 센서를 끕니다. 하부 ToF 여섯 개는 시뮬레이션 차량에 남아 점군을 내지만 아직 이 자율주행의 입력으로 사용하지 않습니다. IMU·엔코더 인터페이스도 남아 있지만 제어 입력은 아닙니다. `simulation.launch.py`의 일반 기본값은 기존 RGB 60·깊이 90 Hz를 유지합니다.
...
RPLIDAR C1의 명목 사양을 참고한 360도·10 Hz·500표본·0.05–12 m 센서입니다. 스캔 평면은 차량 기준 지면에서 약 12.5 cm이며 자기 차체를 피한 **임시** 위치입니다. 낮은 차량 보완용 ToF 링도 모델에 있으나 이 데모 제어에는 아직 연결하지 않았습니다. [C1 단순화 범위](../sensors/RPLIDAR_C1_SIMULATION.md)와 [ToF 링 안내](../sensors/TOF_RING.md)를 참고하십시오.

- 스캔 평면 아래의 낮은 차량·장애물은 라이다가 놓칠 수 있고, 현재 제어기는 ToF 점군을 읽지 않습니다. 하부 센서를 모델에 추가한 것과 다중 차량 안전 제어 구현을 구분합니다.
