...
갱신: 2026-08-30. 이 문서의 실험값은 공식 경기 규격이 아닙니다.
...
| 출발 위치 | 제공된 6개 | 동일한 6개, 그리드 표시 폭만 15 cm로 조정 |
| ID 30 표지판 | 기존 왼쪽 | 지름길 입구 간섭을 피해 반대편. `s`·ID·크기 유지 |
| 기본 실행 여부 | 선택 실행 | 기본 실행 |

본선의 잔디 폭 20 cm씩, 벽 높이 30 cm·두께 5 cm, 마커 크기 10 cm와 과속방지턱 높이는 원본 값을 유지합니다. 잔디를 실제 대회의 연석으로 확정한 것은 아니며, 물리적 벽 사이 폭은 위 노면 폭과 다릅니다.
