...
def main():
    ...
    parser.add_argument("--rates", type=float, nargs="+", default=RATES,
                        help="비교할 후보 거리 기준. 기준선 대조를 위해 0.05와 0.02를 포함합니다.")
    args = parser.parse_args()
    rates = tuple(args.rates)
    if (len(set(rates)) != len(rates) or not {0.05, 0.02}.issubset(rates)
            or any(not math.isfinite(rate) or rate < 0 for rate in rates)):
        parser.error("중복 없는 유한 비음수 값이며 0.05와 0.02를 포함해야 합니다.")
    ...
    shutil.copyfile(Path(__file__), output / "recheck_script_snapshot.py")
