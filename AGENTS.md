...
- 현재 이슈 준비 범위는 **배포 월드의 로드 오류부터 물리 갱신·마커 표시까지의 수정 과정을 설명하는 한 건**입니다. 사용자에게 단일 초안을 먼저 보여 줍니다. 읽는 사람이 우리 시뮬레이터나 이전 수정 이력을 안다고 가정하지 않습니다. 나머지 시설·치수 문제는 `docs/track/OFFICIAL_SOURCE_AUDIT.md` §5의 보류 메모로만 유지하며 운영진 업데이트 후 재확인합니다. 정리한 개별 초안·제출 목록을 다시 만들지 않습니다. 이전 문서는 검증 자료의 `issue_consolidation/before_consolidation.zip`에 보존했습니다.
